    ...
    const linkSections = [
        {
            ...
            links: [
                ...
                { text: "Smart watches", path: '/shop', icon: null },
                { text: "Speakers", path: '/shop', icon: null },
                { text: "Earbuds", path: '/shop', icon: null },
            ]
        },
        {
            title: "Company",
        ...
        },
        {
            ...
            links: [
                ...
                { text: "contact@motocart.com", path: 'mailto:contact@motocart.com', icon: MailIcon },
    ...
    return (
        <footer className="w-full border-t border-slate-200 bg-slate-50">
            <div className="mx-auto max-w-7xl px-4 sm:px-6">
                ...
                <div className="grid gap-10 py-12 lg:grid-cols-[1.3fr_2fr] lg:gap-16 lg:py-16">

                    {/* Brand */}
                    <div>
                        <Link href="/" className="text-3xl font-semibold tracking-tight text-slate-800 sm:text-4xl">
                            <span className="bg-linear-to-r from-indigo-600 to-violet-500 bg-clip-text text-transparent">Moto</span>
                            cart<span className="text-indigo-600">.</span>
                        </Link>
                        <p className="mt-5 max-w-md text-sm leading-relaxed text-slate-500">
                            Welcome to Motocart, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.
                        </p>
                        <div className="mt-6 flex items-center gap-3">
                            {socialIcons.map((item, i) => (
                                <a
                                    ...
                                    className="grid size-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-500 transition hover:-translate-y-0.5 hover:border-indigo-300 hover:text-indigo-600"
                                >
                                    <item.icon />
                                </a>
                            ))}
                    ...
                    {/* Link columns: 2 cols on mobile, 3 from sm */}
                    <div className="grid grid-cols-2 gap-8 text-sm sm:grid-cols-3">
                        {linkSections.map((section, index) => (
                            <div key={index} className={index === 2 ? "col-span-2 sm:col-span-1" : ""}>
                                <h3 className="mb-4 text-xs font-semibold uppercase tracking-wider text-slate-800">
                                    {section.title}
                                </h3>
                                <ul className="space-y-3 text-slate-500">
                                    {section.links.map((link, i) => {
                                        const isInternal = link.path.startsWith('/')
                                        const className = "transition hover:text-indigo-600 wrap-break-word"
                                        return (
                                            <li key={i} className="flex items-start gap-2">
                                                ...
                                                {isInternal ? (
                                                    <Link href={link.path} className={className}>{link.text}</Link>
                                                ) : (
                                                    <a href={link.path} className={className}>{link.text}</a>
                                                )}
                                            </li>
                                        )
                                    })}
                ...
                {/* Bottom bar */}
                <div className="flex flex-col items-center justify-between gap-2 border-t border-slate-200 py-6 text-center text-sm text-slate-500 sm:flex-row sm:text-left">
                    <p>Copyright {new Date().getFullYear()} © Motocart. All Rights Reserved.</p>
                </div>
            </div>
        </footer>
    );
